...
public class Score {
	/*
	* Un score reprÃƒÂ©sente de nombre de clics du joueur sur le nombre de bulles du pallier
	* Dans le fichier texte, les scores sont au format suivant |nombre_bulles-nombre_clics|
	* Exemple : |12-7|
	* Chaque score est sÃƒÂ©parÃƒÂ© par un |
	* Exemple |12-6|23-7|
	* Le | sÃƒÂ©pare les scores entre eux, le - sÃƒÂ©pare le nombre_bulles du nombre_clics
	*/
	...
	/*
	 * DÃƒÂ©compose le score de la ligne passÃƒÂ© en paramÃƒÂ¨tre enlÃƒÂ¨ve cette
	 * information de cette derniÃƒÂ¨re afin de "l'allÃƒÂ©ger", moins lourde pour les
	 * prochains traitement dessus
	 */
	public void recupScore(StringBuilder ligne) {
		// ligne est un string qui reprÃƒÂ©sente tout les scores de l'utilisateur tel qu'il sont dans le fichier texte
		// On va a certain moment tronquer cette variable pour faciliter l'analyse de cette variable les prochaines fois
		ligne.delete(0, 1); //Le premier ÃƒÂ©lement de la ligne est le |, on le supprime 
		...
		while (ligne.charAt(i) != '-') { //Tant qu'on a pas atteinte le sÃƒÂ©parateur -, on lit le nombre de clics
			nombre_clics.append(ligne.charAt(i)); //On l'ajoute ÃƒÂ  la variable temporaire contenante le nombre de clics
			i++; //On incrÃƒÂ©mente la variable qui parcoure la ligne
		}
		i++; //On incrÃƒÂ©mente pour passer le - (voir format d'un score dans le fichier texte)
		while (ligne.charAt(i) != '|') { //MÃƒÂªme chose qu'avec le nombre de clics
			nombre_bulles.append(ligne.charAt(i));
			i++;
		}
		this.nombre_clics = Integer.parseInt(nombre_clics.toString()); //On attribut le nombre de clics au score ÃƒÂ  partire de la variable, il faut transformer le String en int
		this.nombre_bulles = Integer.parseInt(nombre_bulles.toString()); //MÃƒÂªme chose
	 ...
	 * Cette mÃƒÂ©thode n'est pas utilisÃƒÂ© dans le programme final, simplement faite pour les tests / premiÃƒÂ¨re ittÃƒÂ©ration
	 */
	public String toString() {
		...
		// Exemple : |12-7|123-8|, l'affichage restera alignÃƒÂ© dans l'invite de commande
		case 1:
